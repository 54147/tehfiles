...
from fastapi import (
    APIRouter, File, UploadFile, Depends, Request, Response, Query, HTTPException
)
...
from src.api.services.file_service.db_helpers import (
    create_new_file_record, get_last_updated_file_record, optimized_random
)
from src.api.database import get_db
from src.api.settings import settings

from src.api.services.file_service.schema import (
    FileUploadResponse, LongestLinesResponse, LongestLineItem
)
...
async def validate_file(file: UploadFile) -> bool:
    ...
    if not file.content_type.startswith("text/"):
        raise HTTPException(status_code=400, detail="Invalid file format. Only text-based files are allowed.")

    return True


@router.post("/upload/", response_model=FileUploadResponse)
async def upload_file(file: UploadFile = File(...), db=Depends(get_db), storage_handle=Depends(get_storage_handle)):
    """ Uploads text files to storage """
    await validate_file(file)

    try:
        await storage_handle.upload_file(file.file, settings.s3_default_bucket_name, file.filename)
    ...
    create_new_file_record(db, settings.s3_default_bucket_name, file.filename, file.size)
...
async def one_random_line(request: Request, db=Depends(get_db), storage_handle=Depends(get_storage_handle)):
    ...
    data = io.BytesIO()
    storage_handle.client.download_fileobj(Bucket=settings.s3_default_bucket_name, Key=last_upload.key, Fileobj=data)
    data.seek(0)

    if not data:
        logger.error(f"File is missing in the storage: {settings.s3_default_bucket_name}/{last_upload.key}")
        raise HTTPException(status_code=404, detail=f"File is missing in the storage: {last_upload.key}")
...
async def one_random_line_backwards(db=Depends(get_db), storage_handle=Depends(get_storage_handle)):
    ...
    data = io.BytesIO()
    storage_handle.client.download_fileobj(Bucket=settings.s3_default_bucket_name, Key=random_file.key, Fileobj=data)
    data.seek(0)

    if not data:
        logger.error(f"File is missing in the storage: {settings.s3_default_bucket_name}/{random_file.key}")
        raise HTTPException(status_code=404, detail=f"File is missing in the storage: {random_file.key}")
...
async def twenty_longest_lines(random_file: bool = Query(False), db=Depends(get_db), storage_handle=Depends(get_storage_handle)):
    ...
    if random_file:
        ...
        file_to_search = get_last_updated_file_record(db, settings.s3_default_bucket_name)

    if not file_to_search:
        raise HTTPException(status_code=404, detail="No files have been uploaded yet.")

    data = io.BytesIO()
    storage_handle.client.download_fileobj(Bucket=settings.s3_default_bucket_name, Key=file_to_search.key,
                                           Fileobj=data)
    data.seek(0)

    if not data:
        logger.error(f"File is missing in the storage: {settings.s3_default_bucket_name}/{file_to_search.key}")
        raise HTTPException(status_code=404, detail=f"File is missing in the storage: {file_to_search.key}")
